...
本文档说明如何使用 `.github/workflows/windows-jvm-release.yml` 构建 XyMusic 的 Windows JVM 桌面端正式包。该 workflow 基于 `desktopApp` 的 Compose Desktop 配置，明确运行 `:desktopApp:packageReleaseMsi`，不会运行 dev、hot reload、run 或测试任务。
...
3. 如果希望 Windows MSI 和 Android APK/AAB 在同一个 Release 下，两个 workflow 必须从同一个提交运行；两个发布 workflow 会排队执行，避免同时创建同一个 Release。

## 发布步骤

1. 打开 GitHub 仓库的 `Actions` 页面。
2. 选择 `发布 Windows JVM 正式应用`。
3. 点击 `Run workflow`：
   - `release_notes` 可留空，workflow 会生成基础说明。
   - `prerelease` 只在需要预发布时勾选。
4. workflow 成功后会创建或复用当前提交上的 `v<app-versionName>` tag，并把 Windows MSI 上传到 GitHub Release。
...
- 当前 workflow 只生成 Windows MSI；如果后续要发布 macOS DMG 或 Linux DEB，可以参考 `packageReleaseDmg` 和 `packageReleaseDeb` 增加独立 workflow。
