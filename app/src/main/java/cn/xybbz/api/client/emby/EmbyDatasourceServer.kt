...
import cn.xybbz.page.emby.EmbyAlbumOrPlaylistMusicListRemoteMediator
import cn.xybbz.page.emby.EmbyAlbumRemoteMediator
import cn.xybbz.page.emby.EmbyArtistAlbumListRemoteMediator
import cn.xybbz.page.emby.EmbyFavoriteMusicRemoteMediator
import cn.xybbz.page.emby.EmbyGenreAlbumListRemoteMediator
import cn.xybbz.page.emby.EmbyGenresRemoteMediator
import cn.xybbz.page.emby.EmbyMusicRemoteMediator
...
class EmbyDatasourceServer(
    ...
    override suspend fun createApiClient(
        ...
        setToken()
        embyApiClient.setRetrofitData(address)
    }
    ...
    suspend fun searchMusicFlowList(search: String): List<XyMusic> {
        return getServerMusicList(
            startIndex = 0, pageSize = ALBUM_MUSIC_LIST_PAGE_SIZE, search = search
        ).items
    }
...
//                    prefetchDistance = ALBUM_MUSIC_LIST_PAGE_SIZE, // 距离下一页请求的距离
    ...
    override suspend fun importPlaylist(playlistData: ExportPlaylistData): Boolean {
        ...
        if (playlists.isNotEmpty()) {
            val items = embyApiClient.itemApi()
                .getUserItems(
                    userId = connectionConfigServer.getUserId(),
                ...
                playlists.forEach {
                    if (playlistMap.containsKey(it.itemId)) {
                        ...
                        playlists.forEach { playlist ->
                            val playlistId = embyApiClient.playlistsApi().createPlaylist(
                                name = playlist.name
                            ).id
    ...
    override fun setToken() {
        TokenServer.setTokenData(embyApiClient.getToken())
        TokenServer.setHeaderMapData(embyApiClient.getHeadersMapData())
    }
    ...
    fun convertToGenre(genre: ItemResponse): XyGenre {
        ...
        return XyGenre(
            itemId = genre.id,
            pic = itemImageUrl ?: "",
            name = genre.name ?: Constants.UNKNOWN_ALBUM,
            connectionId = connectionConfigServer.getConnectionId(),
