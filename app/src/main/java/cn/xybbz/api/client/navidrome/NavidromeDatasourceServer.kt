...
import cn.xybbz.page.navidrome.NavidromeAlbumOrPlaylistMusicListRemoteMediator
import cn.xybbz.page.navidrome.NavidromeAlbumRemoteMediator
import cn.xybbz.page.navidrome.NavidromeArtistAlbumListRemoteMediator
import cn.xybbz.page.navidrome.NavidromeFavoriteMusicRemoteMediator
import cn.xybbz.page.navidrome.NavidromeGenreAlbumListRemoteMediator
import cn.xybbz.page.navidrome.NavidromeGenresRemoteMediator
import cn.xybbz.page.navidrome.NavidromeMusicRemoteMediator
...
class NavidromeDatasourceServer(
    ...
    override suspend fun createApiClient(
        ...
        navidromeApiClient.setRetrofitData(address)
    }
    ...
    override suspend fun importPlaylist(playlistData: ExportPlaylistData): Boolean {
        ...
        if (playlists.isNotEmpty()) {
            ...
            val playlistServerList = getPlaylistsServer(0, 0)
            //获得新建的表单
            ...
            if (!itemDataList.isNullOrEmpty()) {
                ...
                playlists.forEach { playlist ->
                    if (playlistMap.containsKey(playlist.itemId)) {
                        ...
                        playlists.forEach { playlist ->
                            val playlistId = navidromeApiClient.playlistsApi().createPlaylist(
                                name = playlist.name
                            )?.id
                            val playlistMusics = playlistMusicMap[playlist.itemId]
    ...
    override suspend fun selectArtistInfoById(artistId: String): XyArtist? {
        ...
        if (artistInfo == null) {
            val artist = navidromeApiClient.artistsApi().getArtist(artistId)
            artistInfo = artist?.let { convertToArtist(it, indexNumber = 0) }
        }
        return artistInfo
    }
    ...
    override fun setToken() {
        TokenServer.setTokenData(navidromeApiClient.getToken())
        TokenServer.setQueryMapData(navidromeApiClient.getQueryMapData())
        TokenServer.setHeaderMapData(navidromeApiClient.getHeadersMapData())
    }
    ...
    fun convertToAlbum(album: AlbumItem, ifPlaylist: Boolean = false): XyAlbum {
        return XyAlbum(
            ...
            premiereDate = album.originalDate?.atStartOfDay(ZoneOffset.ofHours(8))?.toInstant()
                ?.toEpochMilli(),
            year = album.maxYear,
