    ...
    ModalBottomSheetExtendComponent(
        ...
        BoxWithConstraints(modifier = Modifier.fillMaxWidth()) {
            Column(
                ...
                XyRow {
                    Text(text = "当前播放列表", fontWeight = FontWeight.W900, fontSize = 19.sp)
                    TextButton(onClick = {
                        //删除数据库内容
                        onClearPlayerList()
                    }) {
                        XyItemText(text = "清除", color = MaterialTheme.colorScheme.primary)
                    }
...
fun MusicList(
    ...
    LazyColumnNotVerticalComponent(
        modifier = Modifier,
        ...
        item { LazyLoadingAndStatus("已经到底了", ifLoading = ifNextPageNumList) }
    }
}
