...
class MainViewModel @Inject constructor(
    ...
    private fun musicControllerInit() {
        ...
        musicController.setOnPauseFun {
            ...
            cacheController.cancelCache(it.musicUrl)
            Log.i("=====", "调用暂停方法")
    ...
    private fun setPlayerProgress(progress: Long) {
        viewModelScope.launch {
            musicController.musicInfo?.let {
                //判断是否需要存储播放历史
                if (_settingsConfig.getSettingsData().ifEnableAlbumHistory || (enableProgressMap.containsKey(
                        it.album
    ...
    val iconList by mutableStateOf(
        listOf(
            PlayerTypeData(icon = Icons.Rounded.RepeatOne, message = "单曲循环"),
            PlayerTypeData(icon = Icons.Rounded.Repeat, message = "列表循环"),
            PlayerTypeData(icon = Icons.Rounded.Shuffle, message = "随机播放"),
        )
    )
