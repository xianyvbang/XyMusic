...
@Inject constructor(
    ...
    val _dataSourceManager = dataSourceManager
     ...
     * 导入歌单= 新增一些歌单
     */
    fun createObject(jsonStr: String) {
        viewModelScope.launch {
            MessageUtils.sendWaitDialog("导入数据读取中...")
            if (jsonStr.isNotBlank()) {
                val moshi = Moshi.Builder()
                    .add(KotlinJsonAdapterFactory())
                    .build()
                val adapter = moshi.adapter(ExportPlaylistData::class.java)
                val backupData = adapter.fromJson(jsonStr)
                if (backupData != null) {
                    dataSourceManager.importPlaylist(backupData)
                    getServerPlaylists()
                }
                MessageUtils.sendTipDialog("歌单导入成功", WaitDialog.TYPE.SUCCESS, 1500)
            } else {
                MessageUtils.sendTipDialog("导入歌单数据为空", WaitDialog.TYPE.ERROR, 1500)
            }
        }
