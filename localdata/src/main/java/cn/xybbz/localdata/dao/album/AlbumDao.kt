...
interface AlbumDao {
     ...
     * 根据id获得专辑信息数据
     */
    @Query("select * from xy_album where itemId = :itemId and connectionId = (select connectionId from xy_settings) ")
    @Transaction
    ...
    @Query(
        """
        select xa.*,(select count(musicId) from playlistmusic where connectionId = (select connectionId from xy_settings) and playlistId = xa.itemId) as musicCount 
        from xy_album xa 
    ...
    @Query(
        """
        select mi.* from maximumplayalbum mpm
        inner join xy_album mi on mpm.albumId = mi.itemId
    ...
    @Query(
        """
        select mi.* from playhistoryalbum phm
        inner join xy_album mi on phm.albumId = mi.itemId
